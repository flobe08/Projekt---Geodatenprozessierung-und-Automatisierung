...
# Unused:
# These classes are kept for validation, but are not automatically buffered.
# They are too broad or too unclear for a first automatic exclusion layer.
WIND_STREET_UNUSED = {
    "road",
    "service",
    "track",
...
def remove_existing_output(output_file: Path) -> None:
    ...
    try:
        ...
        raise SystemExit(
            "Wind OSM street script stopped because the output file is still open."
        ) from error
...
def wind_street_layer_name() -> str:
...
def wind_street_buffer_layer_name() -> str:
    """Return the wind-specific OSM street buffer layer name."""

    return "osm_streets_wind_ausschluss_puffer"


# =============================================================================
# 2. Prepare wind OSM street layer
# =============================================================================
def prepare_wind_osm_streets(municipality_name: str) -> None:
    """Prepare wind-specific OSM street exclusion data."""

    safe_name = safe_filename(municipality_name)
    input_file = OSM_STREETS_DIR / f"{safe_name}_osm_streets.gpkg"
    output_file = OUTPUT_DIR / f"{safe_name}_osm_wind_streets.gpkg"
    buffer_output_file = OUTPUT_DIR / f"{safe_name}_osm_wind_streets_puffer.gpkg"
    ...
    log_info(f"Preparing wind OSM street layer for: {municipality_name}")
    ...
    log_info(f"Wind street exclusion classes: {', '.join(sorted(WIND_STREET_AUSSCHLUSS))}")
    log_info(f"Wind street unused classes: {', '.join(sorted(WIND_STREET_UNUSED))}")
    log_info(
        "Wind street buffer rules: "
        + ", ".join(
            f"{key}={value}m"
            for key, value in sorted(WIND_OSM_STREET_BUFFER_RULES_METERS.items())
        )
    )
    ...
    if streets.empty or "highway" not in streets.columns:
        wind_streets = empty_lines_gdf()
    else:
        wind_streets = streets[streets["highway"].isin(WIND_STREET_AUSSCHLUSS)].copy()

    if not wind_streets.empty:
    ...
    write_layer(buffer_output_file, wind_street_buffer_layer_name(), wind_street_buffers)
