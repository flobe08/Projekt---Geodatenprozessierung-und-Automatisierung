...
2. Read the buffered wind OSM street exclusion layer from wind script 4.
3. Clip both inputs exactly to the municipality boundary.
4. Write the clipped input layers and one combined wind exclusion layer.
"""
...
def build_wind_exclusion_layer(municipality_name: str) -> None:
    ...
    osm_input = wind_osm_buffer_file(municipality_name)
    ...
    log_info(f"Input 2: {display_path(osm_input)}")
